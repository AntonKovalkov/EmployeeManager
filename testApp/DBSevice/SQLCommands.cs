...
namespace testApp.DBSevice
{
    internal struct SQLCommands
    {
        internal struct CreateData
        {
            ...
            public static string InsertEmployee(string name, string lastName, string email, string birthday, string depID, string posID)
            {
                return $"INSERT INTO Employees\r\nVALUES\r\n('{name}', '{lastName}', '{email}', '{birthday}', {depID}, {posID})";
            }
        }
        ...
        internal struct GetData
        {
            ...
            public static string getAll = "SELECT Employees.id, Employees.first_name, Employees.last_name, Employees.email, Employees.birthday, Departments.name, Positions.name FROM Employees\r\nJOIN Departments ON Departments.id = department_id\r\nJOIN Positions ON Positions.id = position_id";
